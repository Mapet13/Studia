...
public:
    TwoDimensionMatrix();
    TwoDimensionMatrix(const TwoDimensionMatrix&) = default;
    TwoDimensionMatrix(const MatrixElement arr[size][size]);
    ...
    MatrixElement get(size_t x, size_t y) const { return matrix[x][y]; }
    ...
    friend std::istream& operator>>(std::istream&, TwoDimensionMatrix&);
    friend std::ostream& operator<<(std::ostream& out, const TwoDimensionMatrix& matrix);

private: 
    void for_each(std::function<void(MatrixElement&)> function);
    void for_each(std::function<void(const MatrixElement&)> function) const;
    void for_each(std::function<void(MatrixElement&, size_t x, size_t y)> function);
